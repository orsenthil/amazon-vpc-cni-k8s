...
func setupDescribeNetworkInterfacesPagesWithContextMock(
	...
	mockEC2.EXPECT().
		...
		Times(times).
		DoAndReturn(func(_ context.Context, _ *ec2.DescribeNetworkInterfacesInput) (*ec2.DescribeNetworkInterfacesOutput, error) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			for _, call := range tt.fields.describeNetworkInterfacePagesCalls {
				mockEC2.EXPECT().
					DescribeNetworkInterfaces(gomock.Any(), call.input).
					DoAndReturn(func(_ context.Context, _ *ec2.DescribeNetworkInterfacesInput) (*ec2.DescribeNetworkInterfacesOutput, error) {
						if call.err != nil {
							return nil, call.err
						}
